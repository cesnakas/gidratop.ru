...
foreach ($arParams['PROPERTY_CODE'] as $code) {
    ...
    if ($ar_props = $db_props->GetNext()) {
        ...
        if($arFile)
        $ar_props["VALUE"]  = '<a class="prop_link prop_tooltip" title="'.GetMessage("OPEN_IN_IN").'" href="'.$arFile['SRC'].'" target="_blank:">'.$arFile['FILE_NAME'].'</a>';
            ...
            if ($prop['PROPERTY_TYPE'] == "E") {
                ...
                if (CModule::IncludeModule("highloadblock")):
                    ...
                    while ($row = $result->Fetch()) {
                        ...
                        $arHB['PICTURE'] = CFile::GetPath($row["UF_FILE"]);
                        $arHB['NAME'] = $row['UF_NAME'];
                        $per_val =$per_val.'<div class="value_item"><div class="flag" style="background-image: url(' . $arHB['PICTURE'] . ');"></div><span class="country">'.$arHB['NAME'].'</span></div>';
                    }
    ...
    foreach ($aTopProp as $key => $prop){
        if ($prop['PROPERTY_TYPE'] == "E"){
            ...
            if (CModule::IncludeModule("highloadblock")):
                ...
                if($arHB)
                $aTopProp[$key]['VALUE'] = '<div class="value_item"><div class="flag" style="background-image: url(' . $arHB['PICTURE'] . ');"></div><span class="country">'.$arHB['NAME'].'</span></div>';
            endif;
        }elseif($prop['PROPERTY_TYPE'] == "G"){
            $res = CIBlockSection::GetByID($prop["VALUE"]);
            if($ar_res = $res->GetNext()){
