...
$bxajaxid = CAjax::GetComponentID($component->__name, $component->__template->__name, 'catalog');
...
<div id="comp_<?=$bxajaxid?>" class="row">
<div class="catalog-section bx-<?=$arParams['TEMPLATE_THEME']?> clearfix" data-entity="<?=$containerName?>">
	<?
		if($arResult['ITEMS'])
...
if($arResult["NAV_RESULT"]->nEndPage > 1 && $arResult["NAV_RESULT"]->NavPageNomer<$arResult["NAV_RESULT"]->nEndPage):?>
    <div class="box-view-more" id="btn_<?=$bxajaxid?>">
        <a data-ajax-id="<?=$bxajaxid?>" href="javascript:void(0)" data-show-more="<?=$arResult["NAV_RESULT"]->NavNum?>" data-next-page="<?=($arResult["NAV_RESULT"]->NavPageNomer + 1)?>" data-max-page="<?=$arResult["NAV_RESULT"]->nEndPage?>">
            <span><?=GetMessage("POKAZ_ESHE");?></span>
        </a>
    </div>
<?endif?>
...
<?if ($showBottomPager)
{
    ...
    <?if(!$arResult['ITEM_ROWS']):?>
        ...
        <div class="catalog-empty row">
            <p><?=Loc::getMessage('K_NOT_PRODUCT')?></p>
        </div>
...
<?if ($arParams['HIDE_SECTION_DESCRIPTION'] !== 'Y'):?>
    <div class="decripshen_section">
       ...
       <?if (!empty($arParams['PODBOKI_TEXT'])){
           echo $arParams['~PODBOKI_TEXT'];
       }else{
