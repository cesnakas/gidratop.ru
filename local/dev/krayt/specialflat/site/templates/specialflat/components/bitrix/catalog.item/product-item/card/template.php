    ...
    <div class="product-item-box">
        <a class="product-item-image-wrapper" title="<?= $imgTitle ?>"
            ...
            <div class="item_lable_blck">
                ...
                <? foreach ($item['LABEL_ARRAY_VALUE'] as $code => $value) { ?>
                    <div class="lable_box <?=mb_convert_case($code,MB_CASE_LOWER )?> <?= (!isset($item['LABEL_PROP_MOBILE'][$code]) ? '' : '') ?>">
        ...
        if (!empty($arParams['PRODUCT_BLOCKS_ORDER'])) {
            ...
            if (!empty($item['PROPERTIES'])) {
                ?>
                <?
                $arPropSection = array($arParams['PROP_1'], $arParams['PROP_2'], $arParams['PROP_3'], $arParams['PROP_4'], $arParams['PROP_5']);
                $arPropSection = array_diff($arPropSection, array(''));
                if (count($arPropSection) > 0) {
                    ?>
                    <div class="product-item-properties" data-entity="props-block">
                        <? $i = 0;
                        foreach ($item['PROPERTIES'] as $code => $displayProperty) {
                            if (!empty($arResult['SECTION_PROP']['UF_PROP_SECTION'])) {
                                foreach ($arResult['SECTION_PROP']['UF_PROP_SECTION'] as $section) {
                                    ?>
                                    <? if ($section == $code) {
                                        ?>
                                        <? if (!empty($displayProperty['VALUE'])) { ?>
                                            <div class="prop-cont">
                            ...
                            } elseif (isset($arPropSection)) {
                                ?>
                                <? foreach ($arPropSection as $section) {
                                    if ($section == $code) {
                                        ?>
                                        <? if (!empty($displayProperty['VALUE'])) { ?>
                                            <div class="prop-line">
                                                            <span class="prop-name"><?= $displayProperty['NAME'] ?>
                                                                :</span>
                                                <span><?= (is_array($displayProperty['VALUE'])
                                                        ? implode(' / ', $displayProperty['VALUE'])
                                                        : $displayProperty['VALUE']) ?>
                                                        </span>
                                            </div>
