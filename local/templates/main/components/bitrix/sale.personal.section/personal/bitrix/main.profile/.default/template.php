...
<div class="bx_profile mobile_wrp">

    <h1><?echo GetMessage("TITLE_MAIN_PRIFILE");?></h1>
	...
	<form method="post" name="form1" action="<?=$APPLICATION->GetCurUri()?>" enctype="multipart/form-data" role="form">
		...
		<p class="main-profile-form-buttons-block col-sm-9 col-md-offset-3">
            ...
            <label class="btn_save label_btn btn">
                <span><?echo GetMessage("SAVE");?></span>
			...
			&nbsp;
            <label class="btn_cansel label_btn btn_anim">
                <span><?echo GetMessage("OTMENA");?></span>
			    <input type="submit" class="btn btn-themes btn-default btn-md"  name="reset" value="<?echo GetMessage("MAIN_RESET")?>">
            </label>
		</p>
	</form>
	<div class="col-sm-12 main-profile-social-block" style="display: none;">
		<?
		if ($arResult["SOCSERV_ENABLED"])
		{
			$APPLICATION->IncludeComponent("bitrix:socserv.auth.split", "special_flat", array(
				"SHOW_PROFILES" => "Y",
				"ALLOW_DELETE" => "Y"
			),
				false
